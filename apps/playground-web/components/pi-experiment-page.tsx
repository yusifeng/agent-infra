...
export function PiExperimentPage() {
  const {
    status,
    error,
    warning,
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-4 p-4 lg:p-6">
      <header className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="space-y-2">
          <h1 className="text-2xl font-semibold text-slate-900">pi browser experiment</h1>
          <p className="max-w-4xl text-sm leading-6 text-slate-600">
            <code>playground-web</code> is now just an experiment harness. <code>pi-agent-core</code> runs the agent loop, <code>pi-ai</code> provides the faux and real models, and a narrow slice of <code>pi-web-ui</code> handles IndexedDB storage plus provider and session dialogs.
          </p>
          <p className="max-w-4xl text-sm leading-6 text-slate-600">
            This route stays browser-local on purpose. It does not write into <code>agent-infra</code> durable persistence, and it is kept separate from the runtime adapter work in <code>packages/</code>.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Link
            href="/runtime-pi"
            className="rounded-md border border-sky-300 bg-sky-50 px-3 py-2 text-sm font-medium text-sky-700 hover:bg-sky-100"
          >
            Open durable runtime test
          </Link>
          <button
            type="button"
            onClick={() => {
      ...
      <section className="flex min-h-[72vh] flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50 p-4">
          ...
          {status === 'error' ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">Failed to start the pi experiment harness.</div> : null}
        ...
        >
          <div className="space-y-3">
            <textarea
              ...
              placeholder="Send a prompt to the pi experiment..."
              disabled={!isReady}
              rows={4}
