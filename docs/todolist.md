...
- [ ] 让 [apps/playground-vite-web/src/features/durable-chat/repo/chat-api.ts](/Users/david/Documents/github/agent-infra/apps/playground-vite-web/src/features/durable-chat/repo/chat-api.ts) 从空壳 re-export 变成真实 facade
- [ ] 将 timeline fetch 收口到 `repo/chat-api.ts`
- [ ] 将 thread messages fetch 收口到 `repo/chat-api.ts`
- [ ] 评估是否将 search panel 相关数据抓取也统一收口到 repo
- [ ] 新建 `repo/browser-clipboard.ts`，承接 `navigator.clipboard` 访问
...
- [ ] 把 live assistant 的可复制文本聚合逻辑下沉到 `service`
- [ ] 把 search label 是否显示、copy scope、action visibility 等规则下沉到 `service`
- [ ] 把 `SearchStatusLabel` 的文案 / 状态映射规则从组件中抽离
- [ ] 把 `LiveAssistantContent` 的 segment 组合顺序、可见性判定抽离成纯函数
...
- [ ] 评估 [apps/playground-vite-web/src/features/durable-chat/components/composer-dock.tsx](/Users/david/Documents/github/agent-infra/apps/playground-vite-web/src/features/durable-chat/components/composer-dock.tsx) 的 toggle / send-button policy 是否要抽成 `service/composer-state.ts`
...
- [ ] 完成 `repo/chat-api.ts` facade 化
- [ ] 将 clipboard 封装挪到 `repo/browser-clipboard.ts`
- [ ] 视需要把 `sessionStorage` 读写也纳入 repo

### 第 4 轮：runtime 变薄

- [ ] 在 `use-durable-chat-runtime.ts` 中只保留 orchestration / side effects
- [ ] 评估是否将 live draft 恢复与轮询拆成 `runtime/live-draft-recovery.ts`
...
- [ ] 为 `SearchResultsPanel` 增加 loading / error / populated 三态测试
